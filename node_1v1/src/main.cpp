...
char data[] = "Hello World from node 1";
...
void loop()
{
  Serial.println("Sending to Gateway_server");
  // led_state();  
  // Send a message to a rf22_mesh_server
  // A route to the destination will be automatically discovered.
  if (manager.sendtoWait((uint8_t *)data, sizeof(data), GATEWAY_ADDRESS) == RH_ROUTER_ERROR_NONE)
  {
    // It has been reliably delivered to the next node.
    // Now wait for a reply from the ultimate server
    
    uint8_t len = sizeof(buf);
    uint8_t from = GATEWAY_ADDRESS;  
